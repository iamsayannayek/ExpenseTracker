...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  const [currentMonth, setCurrentMonth] = useState(() => {
    ...
    // This dynamically gets the actual current year and month (e.g., "2026-06")
  ...
  useEffect(() => {
    ...
    investments.forEach((inv) => {
      if (inv.autoSchedule && inv.nextPaymentDate) {
        const nextMonth = inv.nextPaymentDate.substring(0, 7); // Format: YYYY-MM

        // Only schedule if the date has arrived in the current planner month
        if (nextMonth === currentMonth) {
          ...
          if (!alreadyExists) {
            // If it's an expense policy (LIC), stack the missed premiums. Otherwise, normal amount.
