...
function AllCommitmentsView({ onClose }: { onClose: () => void }) {
  ...
  const filtered = useMemo(() => {
    ...
    // Sort logic: Soonest dates first
    return list.sort(
...
export default function DashboardScreen() {
  ...
  const netWorth = useMemo(() => {
    // 1. Sum all accounts (Banks will add to it, CCs will automatically subtract from it because they are negative)
    const accountTotal = app.accounts.reduce((s, a) => s + a.balance, 0);

    // 2. Sum all investments (PPF, MF, etc.)
  ...
  const upcomingCommitments = useMemo(
    () =>
      app.commitments
        .filter((c2) => !c2.isPaid && !c2.isSkipped) // Hide skipped from dashboard!
      ...
      >
        ...
        <View style={{ marginBottom: 20 }}>
          <SectionHeader
            ...
            actionLabel="See All"
            onAdd={() => app.openCommitmentModal()} // <--- THIS ADDS THE BUTTON
          />
        ...
        <View>
          <SectionHeader
            ...
            actionLabel="See All"
            // onAdd={() => app.openTxModal()} // <--- THIS ADDS THE BUTTON
          />
          {recentTxs.length === 0 ? (
            <View
              style={{
